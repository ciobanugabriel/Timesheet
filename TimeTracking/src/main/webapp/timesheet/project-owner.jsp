...
<fmt:bundle basename="Timesheet">
    <div class="bg-dark my-5 w-50 d-flex justify-content-center text-warning p-3"
        ...
        <form action="project-owner" method="post">
            <div class="m-auto"><h1 class="text-center"><fmt:message key="project-owner"/></h1></div>
        ...
        <form id="ownerForm" action="project-owner" method="post"></form>
    </div>
    <table class="table table-hover table-dark text-warning my-5" style="margin: auto; width: 98%;">
        ...
        <tbody>
        <c:forEach items="${requestScope.projectOwners}" var="project">
            <tr>
                <th scope="row">${project.id}</th>
                <td>${project.name}</td>
                <td class="text-right">
                    <button type="submit" class="btn btn-outline-warning btn-sm mx-2" value="update" name="button"
                            onclick="updateFunction('${project.id}','${project.name}')"><fmt:message key="edit"/>
                    </button>
                    <button type="submit" form="ownerForm" class="btn btn-outline-danger btn-sm mx-2" value="delete"
                            name="button"
                            onclick="deleteFunction('${project.id}')"><fmt:message key="delete"/>
                    </button>
                </td>
            </tr>
        </c:forEach>
...
<script>
    ...
    function deleteFunction(id) {
        ...
        element.setAttribute("name", "ownerID");
